...
from osn_models import ImagenetModels,osn_model
from ldm.util import instantiate_from_config

from Masked_PGD import LinfPGDAttack
from gzhu_utils import parse_args, load_mask, closing_resize, load_image_from_path
...
# class target_osnmodel(nn.Module):
#     """
#     A virtual model which computes the semantic and textural loss in forward function.
#     """

#     def __init__(self, model,osnmodel,
#                  condition: str,
#                  target_info: str = None,
#                  mode: int = 2,
#                  rate: int = 10000,
...
def init(epsilon: int = 16, steps: int = 100, alpha: int = 1,
    ...
    osnmodel=osn_model.OSN_Model(model,object)
    ...
    net.eval()
    # osnnet = target_osnmodel(model,osnmodel, input_prompt, mode=mode, rate=rate)
    osnmodel.eval()
    ...
    return {'net': net, 'osnnet':osnmodel, 'fn': fn, 'parameters': parameters}
...
def infer(img: PIL.Image.Image, config, tar_img: PIL.Image.Image = None, mask: PIL.Image.Image = None) -> np.ndarray:
    ...
    img = np.array(img).astype(np.float32) / 127.5 - 1.0  #归一化
    img = img[:, :, :3]  #确保图片只有三通道
    ...
    net.rate = rate
    #osnnet
    # osnnet.target_info = target_info
    # osnnet.target_size = input_size
    # osnnet.mode = mode
    # osnnet.rate = rate

    label = torch.zeros(data_source.shape).to(device)
    print(net(data_source, components=True))
    print(osnnet(data_source,target_info,mode,rate,input_size,components=True))
    

    # Targeted PGD attack is applied.
    attack = LinfPGDAttack(net,osnnet,target_info,mode,rate,input_size,fn, epsilon, steps, eps_iter=alpha, clip_min=-1.0, targeted=True)
    attack_output = attack.perturb(data_source, label, mask=mask)
    print(net(attack_output, components=True))
    print(osnnet(attack_output,target_info,mode,rate,input_size,components=True))
...
if __name__ == "__main__":
    ...
    block_num = args.block_num #block_num =1
    mode = args.mode
    rate = 10 ** (args.rate + 3) # rate=10000
    mask = args.mask
    resize = args.non_resize
    print(epsilon, steps, input_size, block_num, mode, rate, mask, resize)
    target_image_path = 'GZHU.png'
