...
class EncoderDecoder(AbstractModel):
    def __init__(self, num_classes, num_channels=3, encoder_name='resnet34', isResidual=False, isJPEG=False,resRatio=0.02,qf=92):
        ...
        if self.isJPEG:
            ...
            self.diff_jpeg = DiffJPEG.DiffJPEG(differentiable=True)
        #self.qf_table = np.load('/home/junliu/data/advsamples_trainset/analysis/ori_meandown_pairs.npy')
...
class ConvSCSEBottleneckNoBn(nn.Module):
    def __init__(self, in_channels, out_channels, reduction=2):
        ...
        self.seq = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, padding=1),
            nn.ReLU(inplace=True),
            senet.SCSEModule(out_channels, reduction=reduction, mode='maxout'),
        )
